...
FPS         = 50
ZOOM        = 0.3        # Camera zoom, 0.25 to take screenshots, default 2.7
...
HARD_NEG_REWARD = 100 # For actions like going out or timeout # change from 100 to 120
SOFT_NEG_REWARD = 0.1 # -0.1 # For actions like living
...
def original_reward_callback(env):
    ...
    reward += 2000/len(env.track) * count

    # Negative reward -- newly
    ...
    if env.reward > 1000 or env.reward < -120:
...
def default_reward_callback(env):
    ...
    reward += ((((left | right) & not_visited).sum())+0.25/ factor) # add 10 encourage of visiting new place
    ...
    if env.reward > 3000 or env.reward < -400:
        ...
        if not env.allow_outside: reward,done = env.check_outside(reward,done) # Punish with Hard reward
        reward,done = env.check_timeout(reward,done)
...
class CarRacing(gym.Env, EzPickle):
    ...
    '''
    metadata = {
        'render.modes': ['human', 'rgb_array', 'state_pixels','POS'],
        # 'stpe_info': ["POS"],
    ...
    def render(self, mode='human'):
        ...
        if mode=="rgb_array" or mode=="state_pixels" or mode=="HD":
            ...
            image_data = pyglet.image.get_buffer_manager().get_color_buffer().get_image_data() # Output is <ImageData 96x96> 
            arr = np.fromstring(image_data.get_data(), dtype=np.uint8, sep='') # TODO: defect function by typo
        ...
        if mode=='POS':        
            scroll_x = self.car.hull.position[0] # TODO: may need to flip
            scroll_y = self.car.hull.position[1]
            angle = -self.car.hull.angle
            vel = self.car.hull.linearVelocity
            arr = [scroll_x,scroll_y,angle]
        return np.array(arr)
        # return arr
    ...
    def close(self):
        if self.viewer is not None:
            self.viewer.window_closed_by_user()
